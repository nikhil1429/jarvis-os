...
export default function BodyDoubleTimer() {
  ...
  const timerColor = isRunning
    ? getTimerColor(remaining, duration * 60)
    : "#00f0ff";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          className="font-mono text-5xl font-bold mt-1"
          style={{
            color: timerColor,
        ...
        {isRunning && (
          <div
            className="w-48 h-1.5 rounded-full mx-auto mt-2 overflow-hidden"
            style={{ backgroundColor: "#0d2137" }}
          >
            <div
              className="h-full rounded-full transition-all duration-1000 progress-glow-dot"
              style={{
      ...
      >
        {messages.map((msg, i) => (
          ...
          >
            <p
              className={`font-body text-xs max-w-[85%] px-3 py-2 rounded-lg ${
                msg.role === "user"
                  ? "bg-cyan/10 text-cyan/80"
                  : "glass-card text-text-dim holo-response"
              }`}
            >
              {msg.content}
            </p>
        ...
        >
          <input
            ...
            className={`flex-1 bg-void border rounded-lg px-3 py-2 font-body text-xs text-text
              placeholder:text-text-muted focus:outline-none transition-all ${"border-border focus:border-cyan"}`}
          />
          <button
            onClick={() => {
              handleChat(input);
