...
const toolDeclarations = [
  ...
  },
  {
    ...
    description:
      "Get today's stats: tasks done, streak, energy, check-in status",
    parameters: { type: "OBJECT", properties: {} },
  },
  {
    name: "update_concept_strength",
    description: "Update a concept's mastery strength (0-100)",
    parameters: {
      type: "OBJECT",
      properties: {
      ...
      required: ["conceptId", "strength"],
    },
  },
  {
    name: "get_concept_status",
    description: "Get current strength and review status of a concept",
    parameters: {
      type: "OBJECT",
  ...
  },
  {
    ...
    description: "Get concepts below a strength threshold",
    parameters: {
      type: "OBJECT",
  ...
  {
    name: "save_knowledge",
    description: "Save an insight to the Second Brain",
    parameters: {
      type: "OBJECT",
      properties: {
        text: { type: "STRING" },
  ...
  {
    name: "log_journal",
    description: "Save a voice journal entry",
    parameters: {
      type: "OBJECT",
  ...
  {
    name: "add_commitment",
    description: "Add a new commitment/promise",
    parameters: {
      type: "OBJECT",
      properties: { text: { type: "STRING" }, deadline: { type: "STRING" } },
      required: ["text"],
    },
  },
  {
    name: "navigate_app",
    description: "Navigate to a specific tab or mode in JARVIS",
    parameters: {
      type: "OBJECT",
      properties: {
        target: {
          type: "STRING",
          enum: ["cmd", "train", "log", "dna", "stats", "wins", "settings"],
        },
      },
      required: ["target"],
    },
  ...
  {
    name: "quick_capture",
    description: "Save a quick thought or note",
    parameters: {
      type: "OBJECT",
  ...
  {
    name: "get_mood_data",
    description: "Get recent check-in data (mood, energy, sleep, meds)",
    parameters: {
      type: "OBJECT",
  ...
  },
  {
    ...
    description: "Get energy patterns by time of day and day of week",
    parameters: { type: "OBJECT", properties: {} },
  },
  {
    name: "get_battle_plan",
    description: "Get today's battle plan",
    parameters: { type: "OBJECT", properties: {} },
  },
  {
    name: "engage_deep_reasoning",
    description:
      "For complex questions requiring deep analysis. Delegates to Gemini 2.5 Pro with extended thinking.",
    parameters: {
      type: "OBJECT",
...
function readLS(key) {
  try {
    return JSON.parse(localStorage.getItem("jos-" + key));
  } catch {
    return null;
  }
}
function writeLS(key, val) {
  try {
    ...
    if (totalSize > 4 * 1024 * 1024) {
      console.warn(
        "[GeminiVoice] localStorage over 4MB, skipping write for",
        key,
      );
      return;
    }
  ...
  switch (name) {
    case "complete_task": {
      ...
      if (!done.includes(args.taskId)) {
        ...
        core.taskCompletionTimestamps = [
          ...(core.taskCompletionTimestamps || []).slice(-20),
          new Date().toISOString(),
        ];
        writeLS("core", core);
        window.dispatchEvent(
          new CustomEvent("task:complete", { detail: { taskId: args.taskId } }),
        );
    ...
    case "get_weak_concepts": {
      const concepts = readLS("concepts") || [];
      return concepts
        .filter((c) => (c.strength || 0) < args.threshold)
        .map((c) => ({ id: c.id, name: c.name, strength: c.strength || 0 }));
    }
    ...
    case "navigate_app": {
      window.dispatchEvent(
        new CustomEvent("jarvis-navigate", { detail: { tab: args.target } }),
      );
    ...
    case "get_energy_pattern": {
      ...
      return feelings
        .slice(-30)
        .map((f) => ({ date: f.date, energy: f.energy, mood: f.mood }));
    }
...
export default function useGeminiVoice() {
  ...
  const connectInternal = useCallback(() => {
    ...
    console.log(
      "[GeminiVoice] API key:",
      apiKey ? "SET (" + apiKey.substring(0, 8) + "...)" : "MISSING",
    );
    if (!apiKey) {
    ...
    console.log(
      "[GeminiVoice] Creating WebSocket to:",
      url.substring(0, 80) + "...",
    );
      ...
      if (msg.toolCall) {
        ...
        for (const fc of functionCalls) {
          ...
          if (fc.name === "engage_deep_reasoning") {
            ...
            const result = await executeDeepReasoning(
              args,
    ...
    ws.onclose = (e) => {
      console.log(
        "[GeminiVoice] WebSocket closed, code:",
        e.code,
        "reason:",
        e.reason,
      );
